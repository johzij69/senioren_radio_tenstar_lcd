...
void VS1053::stopSong()
{
    ...
    if (vs1053_info)
        vs1053_info("Song stopped incorrectly!");
...
void VS1053::showstreamtitle(const char *ml)
{
    ...
    { // Streamtitle found
        ...
        if (m_streamTitleHash != hash)
        {
            ...
            if (vs1053_showstreamtitle)
                vs1053_showstreamtitle(sTit + pos);
        }
        free(sTit);
    }
    ...
    { // Advertisement found
        ...
        if (idx1 >= 0 && idx2 > idx1)
        {
            ...
            if (vs1053_commercial)
                vs1053_commercial(sAdv + pos);
            free(sAdv);
        }
    }
}
...
void VS1053::processWebFile()
{
    ...
    if (f_webFileDataComplete && InBuff.bufferFilled() < InBuff.getMaxBlockSize())
    {
        ...
        if (m_f_tts)
        {
            ...
            if (vs1053_eof_speech)
                vs1053_eof_speech(m_lastHost);
        }
        else
        {
            AUDIO_INFO("End of webstream: \"%s\"", m_lastHost);
            if (vs1053_eof_stream)
                vs1053_eof_stream(m_lastHost);
        }
        return;
    }
...
bool VS1053::readPlayListData()
{
    ...
    if (!psramFound() && m_playlistFormat == FORMAT_M3U8)
    {
...
const char *VS1053::parsePlaylist_PLS()
{
    ...
    for (int i = 0; i < lines; i++)
    {
        ...
        { // Title1=Antenne Tirol
            ...
            if (vs1053_showstation)
                vs1053_showstation(plsStationName);
...
//----------------------------------------------------------------------------------------------------------------------
const char *VS1053::parsePlaylist_ASX()
    ...
    if (lines)
    {
        for (uint16_t i = 0; i < lines; i++)
        {
            ...
            if (!f_mediaSeq_found)
            {
                ...
                if (xMedSeq == 0)
                { // mo mediaSeqNr but min 3 times #EXTINF found
                    ;
                }
            }
...
//---------------------------------------------------------------------------------------------------------------------
uint64_t VS1053::m3u8_findMediaSeqInURL()
{ // We have no clue what the media sequence is
    ...
    for (int16_t pos = len; pos >= 0; pos--)
    {
        ...
        {
            while (isdigit(m_playlistContent[linesWithURL[0]][pos]))
                pos--;
            pos++;
            uint64_t a, b, c;
...
//---------------------------------------------------------------------------------------------------------------------
bool VS1053::parseHttpResponseHeader()
{ // this is the response to a GET / request
    ...
    { // outer while
        ...
        else if (startsWith(rhl, "icy-metaint:"))
        {
            ...
            m_metaint = i_metaint;
            if (m_metaint)
                m_f_metadata = true; // stream has metadata
        }
        ...
        else if (startsWith(rhl, "icy-description:"))
        {
            ...
            if (vs1053_icydescription)
                vs1053_icydescription(c_idesc);
        }
...
lastToDo:
    if (m_codec != CODEC_NONE)
    {
        ...
        if (m_playlistFormat != FORMAT_M3U8 && vs1053_lasthost)
            vs1053_lasthost(m_lastHost);
        m_controlCounter = 0;
        m_f_firstCall = true;
    }
...
bool VS1053::connecttoFS(fs::FS &fs, const char *path, uint32_t resumeFilePos)
{
    ...
    if (!audiofile)
    {
        ...
        {
            vTaskDelay(2);
            vs1053_info("Failed to open file for reading");
        }
        return false;
    }
...
int VS1053::read_ID3_Header(uint8_t *data, size_t len)
{
    ...
    { /* read ID3 tag and ID3 header size */
        ...
        { // ID3 not found
            if (!m_f_m3u8data)
                if (vs1053_info)
                    vs1053_info("file has no mp3 tag, skip metadata");
    ...
    { // compute extended header size if exists
        ...
        {
            if (vs1053_info)
                vs1053_info("ID3 extended header");
    ...
    { //  exist another ID3tag?
        ...
        else
        {
            ...
            if (!m_f_m3u8data)
                if (vs1053_info)
                    vs1053_info(m_chbuf);
            if (APIC_seen && vs1053_id3image)
                vs1053_id3image(audiofile, APIC_pos, APIC_size);
            return 0;
        }
    }
    return 0;
